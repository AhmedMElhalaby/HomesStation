...
<div class="panel panel-flat tb_padd">
    <div class="panel-heading">
        <h5 class="panel-title"> {{ trans('dash.products') }} </h5>        
    </div>
    <a class="btn btn-primary" href="{{ route('product.create') }}"> {{ trans('dash.add_new_product') }} </a>
        ...
        <tbody>
            ...
            <tr id="row_{{ $product->id }}">
                ...
                @if(count($product->Images) > 0)
                    <td> <img width="100px" class="img-thumbnail" src="{{ $product->Images[0]->image100 }}" /> </td>
                @else
                ...
                <td class="text-center">
                    <a data-popup="tooltip" title="{{ trans('dash.edit_data') }}" href="{{ route('product.edit',['id' => $product->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                    <a data-popup="tooltip" title="{{ trans('dash.delete_data') }}" onclick="sweet_delete( '{{ route('product.destroy', $product->id) }}', {{ $product->id }} )" class="btn btn-danger" > <i class="icon-bin2"></i> </a>
                </td>
            </tr>
            <?php $count++;?>
            @empty
