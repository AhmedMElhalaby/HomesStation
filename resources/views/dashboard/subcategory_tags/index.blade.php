...
<div class="panel panel-flat tb_padd">
    <div class="panel-heading">
        <h5 class="panel-title"> {{ trans('dash.subcategory_tags') }} </h5>        
    </div>

    <a class="btn btn-primary" href="{{ route('subcategory_tag.create') }}"> {{ trans('dash.add_new_subcategory_tag') }} </a>

    <table class="table table-condensed table-hover datatable-highlight">
        <thead>
            <tr>
                <th class="text-center"> # </th>
                <th class="text-center"> {{ trans('dash.subcategory') }} </th>
        ...
        <tbody>
            <?php $count = 1;?>
            @forelse($subcategory_tags as $subcategory_tag)
            <tr id="row_{{ $subcategory_tag->id }}">
                <td class="text-center"> {{ $count }} </td>
                <td class="text-center"> {{ $subcategory_tag->Subcategory->name_ar }} - {{ $subcategory_tag->Subcategory->name_en }} </td>
                <td class="text-center"> {{ $subcategory_tag->name_ar }} </td>
                <td class="text-center"> {{ $subcategory_tag->name_en }} </td>
                <td class="text-center"> {{ $subcategory_tag->created_at->diffforhumans() }} </td>
                <td class="text-center">
                    <a data-popup="tooltip" title="{{ trans('dash.edit_data') }}" href="{{ route('subcategory_tag.edit',['id' => $subcategory_tag->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                    <a data-popup="tooltip" title="{{ trans('dash.delete_data') }}" onclick="sweet_delete( '{{ route('subcategory_tag.destroy', $subcategory_tag->id) }}', {{ $subcategory_tag->id }} )" class="btn btn-danger" > <i class="icon-bin2"></i> </a>
                </td>
            </tr>
            <?php $count++;?>
            @empty
