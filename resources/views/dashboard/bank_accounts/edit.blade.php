...
<div class="row">
    <div class="col-md-6">
        ...
        <!-- Basic layout-->
        <form action="{{ route('bank_account.update', $bank_account->id) }}" class="form-horizontal" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div class="panel panel-flat">
                <div class="panel-heading">
                    <h5 class="panel-title"> {{ trans('dash.bank_accounts.edit_bank_account') }} : {{ $bank_account['name_' . app()->getLocale()] }} </h5>                    
                </div>
                <div class="panel-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-9">
                            <input type="text" class="form-control" name="bank_name" value="{{ $bank_account->bank_name }}" placeholder="{{ trans('dash.bank_accounts.bank_name') }}" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-3 control-label">{{ trans('dash.bank_accounts.owner_account') }}</label>
                        <div class="col-lg-9">
                            <input type="text" name="owner_account" class="form-control" value="{{ $bank_account->owner_account }}" placeholder="{{ trans('dash.bank_accounts.owner_account') }}" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-3 control-label">{{ trans('dash.bank_accounts.account_number') }}</label>
                        <div class="col-lg-9">
                            <input type="number" name="account_number" class="form-control" value="{{ $bank_account->account_number }}" placeholder="{{ trans('dash.bank_accounts.account_number') }}" required>
                        </div>
                    </div>                   
