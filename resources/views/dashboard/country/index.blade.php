...
<div class="panel panel-flat tb_padd">
    ...
    <a class="btn btn-primary" href="{{ route('country.create') }}"> {{ trans('dash.add_new_country') }} </a>
    ...
    <table class="table table-condensed table-hover datatable-highlight">
        <thead>
            <tr>
                ...
                <th class="text-center"> {{ trans('dash.phonecode') }} </th>
        ...
        <tbody>
            ...
            <tr id="row_{{ $country->id }}">
                ...
                <td class="text-center">
                    <a data-popup="tooltip" title="{{ trans('dash.edit_data') }}" href="{{ route('country.edit',['id' => $country->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                    <a data-popup="tooltip" title="{{ trans('dash.delete_data') }}" onclick="sweet_delete( '{{ route('country.destroy', $country->id) }}', {{ $country->id }} )" class="btn btn-danger" > <i class="icon-bin2"></i> </a>
                </td>
            </tr>
            @empty
            @endforelse
