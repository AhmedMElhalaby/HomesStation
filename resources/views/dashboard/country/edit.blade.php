...
<div class="row">
    <div class="col-md-6">
        ...
        <!-- Basic layout-->
        <form action="{{ route('country.update', $country->id) }}" class="form-horizontal" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div class="panel panel-flat">
                ...
                <div class="panel-body">
                    <div class="form-group">
                        ...
                        <div class="col-lg-9">
                            <input type="text" class="form-control" value="{{ $country->name_ar }}" name="name_ar" placeholder="{{ trans('dash.name_ar') }}" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-3 control-label">{{ trans('dash.name_en') }}</label>
                        <div class="col-lg-9">
                            <input type="text" name="name_en" value="{{ $country->name_en }}" class="form-control" placeholder="{{ trans('dash.name_en') }}" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-3 control-label">{{ trans('dash.phonecode') }}</label>
                        <div class="col-lg-9">
                            <input type="text" name="phonecode" value="{{ $country->phonecode }}" class="form-control" placeholder="{{ trans('dash.phonecode') }}" >
                        </div>
                    </div>
