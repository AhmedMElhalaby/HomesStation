...
<div class="row">
    <div class="col-md-6">
        ...
        <!-- Basic layout-->
        <form action="{{ route('subcategory.update', $subcategory->id) }}" class="form-horizontal" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div class="panel panel-flat">
                <div class="panel-heading">
                    <h5 class="panel-title"> {{ trans('dash.edit_subcategory') }} : {{ $subcategory['name_' . app()->getLocale()] }} </h5>                    
                </div>
                <div class="panel-body">
                    <div class="form-group">
                        ...
                        <div class="col-lg-3">
                            <img src="{{ $subcategory->image200 }}" class="img-thumbnail" alt="Cinque Terre">
                        </div>
                    </div>
                    <div class="form-group">
                            ...
                            <select name="category_id" class="select-border-color border-warning" >
                                @foreach ($categories as $category)
                                    @if($category->id == $subcategory->category_id)
                                        <option value="{{ $category->id }}" selected> {{ $category->name_ar . ' - ' .$category->name_en }} </option>                                
                                    @else
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-9">
                            <input type="text" value="{{ $subcategory->name_ar }}" class="form-control" name="name_ar" placeholder="{{ trans('dash.name_ar') }}" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-3 control-label">{{ trans('dash.name_en') }}</label>
                        <div class="col-lg-9">
                            <input type="text" value="{{ $subcategory->name_en }}" name="name_en" class="form-control" placeholder="{{ trans('dash.name_en') }}" required>
                        </div>
                    </div>
