...
<div class="panel panel-flat tb_padd">
    <div class="panel-heading">
        <h5 class="panel-title"> {{ trans('dash.subcategories') }} </h5>        
    </div>

    <a class="btn btn-primary" href="{{ route('subcategory.create') }}"> {{ trans('dash.add_new_subcategory') }} </a>
    ...
    <table class="table table-condensed table-hover datatable-highlight">
        ...
        <tbody>
            ...
            <tr id="row_{{ $subcategory->id }}">
                ...
                <td> <img width="100px" class="img-thumbnail" src="{{ $subcategory->image200 }}" /> </td>
                <td class="text-center"> {{ $subcategory->name_ar }} </td>
                <td class="text-center"> {{ $subcategory->name_en }} </td>
                <td class="text-center"> {{ $subcategory->created_at->diffforhumans() }} </td>
                <td class="text-center">
                    <a data-popup="tooltip" title="{{ trans('dash.edit_data') }}" href="{{ route('subcategory.edit',['id' => $subcategory->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                    <a data-popup="tooltip" title="{{ trans('dash.delete_data') }}" onclick="sweet_delete( '{{ route('subcategory.destroy', $subcategory->id) }}', {{ $subcategory->id }} )" class="btn btn-danger" > <i class="icon-bin2"></i> </a>
                </td>
            </tr>
            <?php $count++;?>
            @empty
