...
<div class="panel panel-flat tb_padd">
    ...
    <a class="btn btn-primary" href="{{ route('nationality.create') }}"> {{ trans('dash.add_new_nationality') }} </a>
    <table class="table table-condensed table-hover datatable-highlight">
        ...
        <tbody>
            ...
            <tr id="row_{{ $nationality->id }}">
                <td class="text-center"> {{ $nationality->name_ar }} </td>
                <td class="text-center"> {{ $nationality->name_en }} </td>
                <td class="text-center"> {{ $nationality->created_at->diffforhumans() }} </td>
                <td class="text-center">
                    <a data-popup="tooltip" title="{{ trans('dash.edit_data') }}" href="{{ route('nationality.edit',['id' => $nationality->id ]) }}" class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                    <a data-popup="tooltip" title="{{ trans('dash.delete_data') }}" onclick="sweet_delete( '{{ route('nationality.destroy', $nationality->id) }}', {{ $nationality->id }} )" class="btn btn-danger" > <i class="icon-bin2"></i> </a>
                </td>
            </tr>
            @empty
            @endforelse            
