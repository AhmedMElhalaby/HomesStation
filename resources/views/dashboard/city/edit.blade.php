...
<div class="row">
    <div class="col-md-6">
        <!-- Basic layout-->
        <form action="{{ route('city.update', $city->id) }}" class="form-horizontal" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div class="panel panel-flat">
                <div class="panel-heading">
                    <h5 class="panel-title"> {{ trans('dash.edit_city') }} : {{ $city['name_' . app()->getLocale()] }}</h5>                    
                </div>
                <div class="panel-body">
                    <div class="form-group">
                        <label class="col-lg-3 control-label">{{ trans('dash.name_ar') }}</label>
                        <div class="col-lg-9">
                            <input type="text" class="form-control" value="{{ $city->name_ar }}" name="name_ar" placeholder="{{ trans('dash.name_ar') }}" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-3 control-label">{{ trans('dash.name_en') }}</label>
                        <div class="col-lg-9">
                            <input type="text" name="name_en" value="{{ $city->name_en }}" class="form-control" placeholder="{{ trans('dash.name_en') }}" required>
                        </div>
                    </div>
                    <div class="form-group">
                            ...
                            <select name="country_id" class="select-border-color border-warning" >
                                ...
                                @foreach ($countries as $country)
                                    @if($country->id == $city->country_id)
                                        <option value="{{ $country->id }}" selected> {{ $country->name_ar.' - '.$country->name_en }} </option>
                                    @else
