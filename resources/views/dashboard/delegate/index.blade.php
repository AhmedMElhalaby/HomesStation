...
<div class="panel panel-flat tb_padd">
    <div class="panel-heading">
        <h5 class="panel-title"> {{ trans('dash.delegates') }} </h5>        
    </div>

    <a class="btn btn-primary" href="{{ route('delegate.create') }}"> {{ trans('dash.add_new_delegate') }} </a>
    ...
    <table class="table table-condensed table-hover datatable-highlight">
        ...
        <tbody>
            ...
            <tr id="row_{{ $delegate->id }}">
                <td class="text-center"> <img width="100px" class="img-thumbnail" src="{{ $delegate->image400 }}" /> </td>
                <td class="text-center"> {{ $delegate->username }} </td>
                <td class="text-center"> {{ $delegate->mobile }} </td>
                <td class="text-center">
                    @if($delegate->banned == '1') 
                ...
                <td class="text-center"> {{ $delegate->City == null ? trans('dash.empty') : $delegate->City['name_'. app()->getLocale()] }} </td>
                <td class="text-center"> {{ $delegate->created_at->diffforhumans() }} </td>
                <td class="text-center">
                    <a href="{{ route('delegate.show', $delegate->id) }}" class="btn btn-success" data-popup="tooltip" title="{{ trans('dash.show_data') }}"><i class="fa fa-tv"></i></a>
                    <a href="{{ route('delegate.edit',['id' => $delegate->id ]) }}" data-popup="tooltip" title="{{ trans('dash.edit_data') }}"  class="btn btn-primary"> <i class="icon-pencil3"></i> </a>
                    {{--  <a data-popup="tooltip" title="archive" class="btn btn-info" > <i class="fa fa-archive"></i> </a>  --}}
                    <a data-popup="tooltip" title="{{ trans('dash.delete_data') }}" onclick="sweet_delete( '{{ route('delegate.destroy', $delegate->id) }}', {{ $delegate->id }} )" class="btn btn-danger" > <i class="icon-bin2"></i> </a>
                </td>
            </tr>
            @empty
            @endforelse
