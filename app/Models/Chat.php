...
class Chat extends Model
{
    ...
    public function setMessageAttribute($value){
        if ($this->attributes['message_type'] == "image") {
            ...
            $value->move(storage_path('app/uploads/chat/voice_message'), $filename);
            $this->attributes['message'] = $filename;
        } else {
    ...
    {
        if ($this->attributes['message_type'] == "image") {
            if (!file_exists(storage_path('app/uploads/chat/images/300' . "/" . $this->attributes['message']))) {
                return trans('app.deleted_file');
            }
            return asset('storage/app/uploads/chat/images/300') . '/' . $this->attributes['message'];
        } else if($this->attributes['message_type'] == "voice_message") {
            if (!file_exists(storage_path('app/uploads/chat/voice_message' . "/" . $this->attributes['message']))) {
                return trans('app.deleted_file');
            }
