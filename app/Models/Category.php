...
class Category extends Model
{
    ...
    public function setImageAttribute($value)
    {
        if (isset($this->attributes['image']) && $this->attributes['image'] != '') {
            if (file_exists(public_path('storage/app/uploads/category/org' . "/" . $this->attributes['image']))) {
    ...
    public function getImage200Attribute()
    {
        if ($this->attributes['image'] != "") {
            if (!file_exists(public_path('storage/app/uploads/category/200' . "/" . $this->attributes['image']))) {
    ...
    {
        if ($this->attributes['image'] != "") {
            if (!file_exists(public_path('storage/app/uploads/category/400' . "/" . $this->attributes['image']))) {
                return asset('storage/app/uploads/default.png');
            }
    ...
    {
        if ($this->attributes['image'] != "") {
            if (!file_exists(public_path('storage/app/uploads/category/600' . "/" . $this->attributes['image']))) {
                return asset('storage/app/uploads/default.png');
            }
